...
void ourparse(){
	while(!halt_flag){
		...
		case 7:
			...
			// our logic
			// ½ÓÊÕÓ²¼þ²é±íÃ»ÃüÖÐµÄ°ü£¬ÐÞ¸Ä¿ØÖÆÆ÷ºÍnetmagicÖÐµÄ×ª·¢±í£¬¹¹ÔìtypeÎª8µÄÊý¾Ý°ü£¬·¢ËÍ¸ønetmagic
			uint16_t outport = 0;
			...
			PacketDigest d;
			uint8_t * ptr = (uint8_t *) (msg.content + 34 + sizeof(struct nmac_header) + 4);// Ö¸ÏòÕý³£ÒÔÌ«Íø±¨ÎÄµÄmacÍ·²¿
			memcpy(&d.dmac, ptr, 6);
